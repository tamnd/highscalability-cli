...
describes what users get. Pick a format with `-o`, or let hsc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
hsc <command> -o table   # aligned columns for reading
hsc <command> -o jsonl   # one JSON object per line, for piping
hsc <command> -o json    # a single JSON array
hsc <command> -o csv     # spreadsheet friendly
hsc <command> -o tsv     # tab-separated
hsc <command> -o url     # just the URL column
hsc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
hsc <command> --fields id,title,url
```
...
```bash
hsc <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
hsc <command>            # a table, because this is a terminal
hsc <command> | wc -l    # JSONL, because this is a pipe
```
