// Package cli builds the hsc command tree on top of the highscalability library.
package cli
...
import (
	...
	"github.com/tamnd/highscalability-cli/pkg/render"
)
...
type App struct {
	client *highscalability.Client
	cfg    highscalability.Config
...
func Root() *cobra.Command {
	app := &App{cfg: highscalability.DefaultConfig()}

	root := &cobra.Command{
		Use:   "hsc",
		Short: "Browse High Scalability architecture articles",
		Long: `hsc reads the High Scalability blog feed and article pages.
No API key required; all data is publicly available.

hsc is an independent tool and is not affiliated with High Scalability or Todd Hoff.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVarP(&app.output, "output", "o", "auto", "output format: table|json|jsonl|csv|tsv|url|raw (auto=table on TTY, jsonl piped)")
...
func (a *App) setup() error {
	...
	a.client = highscalability.NewClient(a.cfg)
	return nil
}
